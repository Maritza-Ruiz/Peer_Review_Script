...
#Maritza comment: Great RQ question and hypothesis. One small suggestion would be to rewrite with slightly more clarity. See suggestion below:

RQ1: Does sleep deprivation mediate the relationship between daily screen time and mental health outcomes?

H1: Greater screen time is associated with reduced sleep duration, which subsequently predicts poorer mental health.
...
#Maritza suggestion: added axis labels
#adjusted labels to make main title bold
...
#Maritza comment: Changed model to method in geom_smooth code

![](images/mediation_analysis_diagram-01.png)

Based on the result, H1 was supported. To better understand the relationship, we draw a path analysis figure. From this figure we can see that overall, longer screen-time usage significantly harms mental health (coeff = −0.70). This also supports our hypothesis that sleep duration partially mediates this relationship: excessive screen time reduces sleep, which in turn contributes to poorer mental health. However, this is not the whole story. Even after accounting for sleep and ensuring adequate sleep duration, screen overuse still shows a significant direct negative effect on mental health (direct coeff= −0.57), suggesting that excessive screen use may harm mental health through additional mechanisms beyond sleep.

#Maritza suggestion (revised summary of above): Based on the results, H1 was supported. To further examine the relationship, we conducted a path analysis. The analysis shows that longer screen-time usage significantly harms mental health (coeff = −0.70). This finding supports our hypothesis that sleep duration partially mediates the relationship: excessive screen time reduces sleep, which in turn contributes to poorer mental health. However, the mediation is only partial. Even after accounting for sleep duration, screen overuse still exerts a significant direct negative effect on mental health (direct coeff = −0.57), indicating that excessive screen use may impair mental health through additional mechanisms beyond sleep.
...
#Maritza suggestion: .groups = "drop" instead of ungroup()
...
#Maritza suggestion (revision of section above): An initial review of the data indicated that individuals who primarily use Instagram, KakaoTalk, Snapchat, TikTok, WeChat, or WhatsApp tend to report below-average mental health. To determine whether these differences represent a statistically significant relationship rather than random variation, we conducted a chi-square test of independence.
...
students <- sm |>
  ...
  drop_na(region) |> # Select the columns to use.
  select(student_id,
         region,
         academic_level,
         age,
